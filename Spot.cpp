...
int main ( int argc, char** argv ) try {  // argc, number of cmd line args; 
    ...
    double incl_1(90.0),               // Inclination angle of the observer, in degrees
           ...
           E_mono(1.0),				   // Monochromatic energy set to 2 keV by default.
    ...
    unsigned int NS_model(1),       // Specifies oblateness (option 3 is spherical)
                 ...
                 atmosphere(0),        // Atmosphere model choice (initialized to hydrogen)
    ...
    bool incl_is_set(false),         // True if inclination is set at the command line (inclination is a necessary variable)
    	 ...
    	 E_mono_set(false),			 // True if monochromatic lightcurve energy is set
...
/*	time_t tim;
    ...
    for ( int i(1); i < argc; i++ ) {
        if ( argv[i][0] == '-' ) {  // the '-' flag lets the computer know that we're giving it information from the cmd line
            switch ( argv[i][1] ) {
	            ...
	            case 'G': // Spectral Model, atmosphere (hydrogen or helium)
	            	sscanf(argv[i+1], "%u", &atmosphere);
	            	break;
	            ...
	            case 'M': // Monochromatic energy band, in keV
	            	sscanf(argv[i+1], "%lf", &E_mono);
	            	E_mono_set = true;
	            	break;
                ...
                case 'h': default: // Prints help
       	            std::cout << "\n\nSpot help:  -flag description [default value]\n" << std::endl
                              ...
                              << "-G Atmospheric model: 0 = hydrogen, 1 = helium. [0]" << std::endl
		                      ...
		                      << "-M Monochromatic energy value, in keV [1.0]" << std::endl
    ...
    if ( !E_mono_set){
     	throw( Exception(" Need to set valid monochromatic energy value for flux calculation. Exiting.\n") );
    	return -1;
    ...
    if ( atmosphere != 0 && atmosphere != 1 ) {
    	throw( Exception (" Invalid atmosphere choice. Existing.\n") );
    	return -1;
    }
    ...
    curve.flags.atmosphere = atmosphere;
	...
	// printing the temperature mesh, for testing
	 /* std::cout << "Temperature Mesh of Spot:" << std::endl;
    	for (unsigned int i(0); i < numtheta; i++) {
    		for (unsigned int j(0); j < numphi; j++) {
    	    	std::cout << T_mesh[i][j] << "  ";
...
*/
    ...
    if ( beaming_model == 0 )
        ...
        out << "% Limb Darkening for Atmospheric models (H/He) " << std::endl;
    if ( normalize_flux )
    ...
    out << "%\n"
        ...
        << "% Column 2: Bolometric Number flux, calculated exactly for blackbody and from 0.01 to 10 keV for atmospheric models, photons/(cm^2 s) \n"
        << "% Column 3: Monochromatic Number flux (photons/(cm^2 s keV) measured at energies (at infinity) of " << E_mono << " keV\n" 
