...
E1Lower=3  # in keV; lower bound of first energy band
E1Upper=80  # in keV; upper bound of first energy band
E2Lower=2  # in keV; lower bound of second energy band
E2Upper=7  # in keV; upper bound of second energy band
...
gnu_file="$exe_dir/output/plots/gplot_${day}_${tele}.txt"    # name of the text file storing gnuplot commands
lightcurve="$exe_dir/output/plots/lightcurve_${day}_${tele}" # the lightcurve graphics output file
tex_file="$exe_dir/output/plots/plot_${day}_${tele}.tex"     # TeX file output with the parameters in the comment
...
echo "set key bottom right;" >> "$gnu_file"
...
#if test "$fluxtype" -eq 2
#	then echo "set label 'Bolometric' at 0.98,0.83 right;" >> "$gnu_file"
#fi
#if test "$fluxtype" -eq 3
...
out_file="${out_dir}"/"${day}"_"${gray_type}"_"${tele}".txt
...
echo -n "'$out_file' u 1:$fluxtype w lines lw 4 title '${tele} $E1Lower-$E1Upper keV'; " >> "$gnu_file"



# Plotting just the output file
#fluxtype=2
#echo -n "'$out_file' u 1:$fluxtype w lines lw 4 title 'Bolometric', " >> "$gnu_file"
#fluxtype=4
#echo -n "'$out_file' u 1:$fluxtype w lines lw 4 title '$E1Lower-$E1Upper keV', " >> "$gnu_file"
#fluxtype=5
#echo -n "'$out_file' u 1:$fluxtype w lines lw 4 title '$E2Lower-$E2Upper keV' ; " >> "$gnu_file"
...
echo "\includegraphics[width=4in]{lightcurve_${day}_highbands.eps}" >> "$tex_file"
echo "\caption[Short here]{\label{fig:LABELHERE} X-ray light curve for $NSshape neutron star with mass \($mass\Msun\), radius $radius~km, inclination angle \($inclination\degrees\), spot emission angle \($emission\degrees\), spin frequency $spin Hz, phase shift $phaseshift, distance $distance m, spot angular radius \($rho\degrees\), $numtheta \(\times\) $numtheta spot mesh, and spot temperature $temp keV. First energy band $E1Lower-$E1Upper keV, second energy band $E2Lower-$E2Upper keV.}" >> "$tex_file"
