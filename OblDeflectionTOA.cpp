...
double OblDeflectionTOA_psi_integrand_wrapper ( double r, bool *prob ) {
	double integrand( OblDeflectionTOA_object->psi_integrand( OblDeflectionTOA_b_value, r )); //OblDeflectionTOA_object is looking up psi_integrand by feeding it OblDeflectionTOA_b_value, which is defined in psi_outgoing
...
/*******************************************************************************/
double OblDeflectionTOA::bmin_ingoing ( const double& cos_theta ) const { 
  	costheta_check( cos_theta );

  	// Need to return the value of b corresponding to dR/dtheta of the surface.
  	double r ( modptr->R_at_costheta( cos_theta ) );    //*** is this the same as rspot? *****
  	double drdth ( modptr->Dtheta_R( cos_theta ) );
	double b ( r / sqrt( (1.0 - 2.0 * get_mass() / r) + pow( drdth / r , 2.0 ) ) );
  	//std::cout << r << std::endl;
...
double OblDeflectionTOA::rcrit ( const double& b, const double& cos_theta, bool *prob ) const {
  	...
  	if ( b > bmax_outgoing(r) || b < bmin_ingoing(cos_theta) ) {
    	std::cerr << "ERROR in OblDeflectionTOA::rcrit(): b out-of-range." << std::endl;
    	*prob=true;
    	dummy=-7888.0;
    	return dummy;
  	}
  ...
  /*	if ( fabs(candidate - modptr->R_at_costheta(1.0) ) <= std::numeric_limits< double >::epsilon()
         || fabs(candidate - modptr->R_at_costheta(0.0) ) <= std::numeric_limits< double >::epsilon() ) { // this indicates no soln
      	...
      	std::cerr << "ERROR in OblDeflectionTOA::rcrit(): returned no solution?" << std::endl;
      	*prob=true;
      	//candidate=-7888.0;
      	candidate/=0.0;
      	return candidate;
...
double OblDeflectionTOA::psi_outgoing ( const double& b, const double& rspot,
  	...
  	if ( fabs( b - b_max ) < 1e-7 ) { // essentially the same as b=b_max
    	...
    	if ( b != 0.0 ) {
      		...
      		psi = Integration( rsurf, get_rfinal(), OblDeflectionTOA_psi_integrand_wrapper ); // integrating from r_surf to ~infinity
    	}				
    	return psi;
  	}
}
...
double OblDeflectionTOA::psi_max_outgoing ( const double& b, const double& rspot, bool *prob ) {
  	...
  	double psi = Integration( rsurf, get_rfinal(), OblDeflectionTOA_psi_integrand_wrapper );
...
double OblDeflectionTOA::psi_ingoing ( const double& b, const double& cos_theta, bool *prob ) const {
  	...
  	if ( b > bmax_outgoing(rspot) || b < bmin_ingoing( cos_theta ) ) {
...
/*****************************************************/
bool OblDeflectionTOA::b_from_psi ( const double& psi, const double& cos_theta, double& b,
  ...
  //bcand;
    ...
    */
    	...
    	if ( ingoing_allowed ) {
      		//std::cout << "ingoing allowed!" << std::endl;

      		bmin_in = bmin_ingoing( cos_theta );
...
double OblDeflectionTOA::dpsi_db_outgoing( const double& b, const double& rspot, bool *prob ) {
  	...
  	rsurf = rspot;
  	double dpsidb = Integration( rsurf, get_rfinal(), OblDeflectionTOA_dpsi_db_integrand_wrapper );
...
double OblDeflectionTOA::dpsi_db_ingoing( const double& b, const double& cos_theta, bool *prob ) {
  	...
  	if ( b > bmax_outgoing(r) || b < bmin_ingoing(cos_theta) ) {
    	std::cerr << "ERRROR in OblDeflectionTOA::dpsi_db_ingoing(): b out-of-range." << std::endl;
    	*prob = true;
    	dummyb = -7888.0;
    	return dummyb;
  	}
...
double OblDeflectionTOA::toa_outgoing ( const double& b, const double& rspot, bool *prob ) {
	// costheta_check(cos_theta);
  	double dummy;
  	if ( b > bmax_outgoing(rspot) || b < 0.0 ) { // 07-Feb-2013: Abbie cleaned up parentheses here
    	std::cerr << "ERROR in OblDeflectionTOA::toa_outgoing(): b out-of-range." << std::endl;
    	std::cerr << "b = " << b << ", bmax = " << bmax_outgoing(rspot) << ", |b - bmax| = " << fabs(b - bmax_outgoing(rspot)) << std::endl;
    	*prob = true;
    	dummy = -7888.0;
    	return dummy;
...
double OblDeflectionTOA::toa_ingoing ( const double& b, const double& cos_theta, bool *prob ) {
  	...
  	if ( b > bmax_outgoing(cos_theta) || b < bmin_ingoing(cos_theta) ) {
  	...
  	// double toa_in = 2.0 * Integration( rcrit, modptr->R_at_costheta(cos_theta),
  	//				                      OblDeflectionTOA_toa_integrand_wrapper,
  	...
  	//std::cout << " b = " << b << " \nrcrit = " << rcrit << " \ntoa_in = " << toa_in << 
				//" \ntoa_in = " << rcrit/sqrt(1-2.0*get_mass()/rcrit) * 2.0*sqrt( 2.0*(modptr->R_at_costheta(cos_theta) - rcrit)/(rcrit-3.0*get_mass()))
...
double OblDeflectionTOA::dpsi_db_integrand ( const double& b, const double& r ) const { 
  	double integrand( (1.0 / ( r * r * sqrt( 1.0 - pow( b / r , 2.0 ) * (1.0 - 2.0 * get_mass() / r) )) ) + (b * b * (1.0 - 2.0 * get_mass() / r) / (pow( r , 4.0 ) * pow( 1.0 - pow( b / r , 2.0 ) * (1.0 - 2.0 * get_mass() / r) , 1.5) ) ) );
  	return integrand;
}
