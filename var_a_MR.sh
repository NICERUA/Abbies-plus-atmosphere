...
# This script varies R, so that we are effectively varying the compactness of NS.
...
echo "Varying R, effectively NS compactness: Mass/Radius"
...
temp=0.10 	   # in keV, if temp < 0.0385 keV (10^5.65K), change bolometric flux integration upper bound to 14.6754
distance=1.41941169e20 # 4.6 kpc in meters

# Writing file names
out_dir="$exe_dir/output/var/var_a_MR"
if test "$gray" -eq 0
	then out_file="${out_dir}"/"${day}"_"${radius}"_bb.txt #blackbody
else
	out_file="${out_dir}"/"${day}"_"${radius}"_"${atmo_type}".txt #h or he
fi
...
cp /Users/kitung/Desktop/thesis_material/Albert_codes/H_He_integrated/output/var/var_a_MR/"${day}"_* /Users/kitung/Desktop/thesis_material/Albert_codes/plotting_output_spectra/plot_with_variables/MR
