...
//numInputs is the number of input pointers, i.e., the number of variables passed to the function
    ...
    char everything_out_file[256],
    	 low_out_file[256],
/***************************************************************************************/
		 /* Need to change filenameheader here below: */
    	 filenameheader[]="Runx";
    	 // need to also change this in init.m, outputFerret.m, and postProcessing.m
/***************************************************************************************/
   ...
   /* time_t tim;
	...
	if ( numInputs != 26 ) { // need to go through and figure out how many inputs we should have
		mexPrintf("numInputs = %d, expecting 26", numInputs);
		mexErrMsgTxt("Wrong number of inputs.");
	}
	
	numbins = mxGetScalar(theInput[0]);
	obsdata.t = mxGetPr(theInput[1]);
	NS_model = mxGetScalar(theInput[2]);
	mass = mxGetScalar(theInput[3]);
	rspot = mxGetScalar(theInput[4]); // note that this is rspot, not req!
	omega = mxGetScalar(theInput[5]);
	incl_1 = mxGetScalar(theInput[6]);
	theta_1 = mxGetScalar(theInput[7]);
	rho = mxGetScalar(theInput[8]);
	spot_temperature = mxGetScalar(theInput[9]);
	E_band_lower_1 = mxGetScalar(theInput[10]);
	E_band_upper_1 = mxGetScalar(theInput[11]);
	E_band_lower_2 = mxGetScalar(theInput[12]);
	E_band_upper_2 = mxGetScalar(theInput[13]);
	aniso = mxGetScalar(theInput[14]);
	ts = mxGetScalar(theInput[15]);
	bbrat = mxGetScalar(theInput[16]);
	gray_flag = mxGetScalar(theInput[17]);
	Gamma1 = mxGetScalar(theInput[18]);
	Gamma2 = mxGetScalar(theInput[19]);
	Gamma3 = mxGetScalar(theInput[20]);
	distance = mxGetScalar(theInput[21]);
	obsdata.f[1] = mxGetPr(theInput[22]);
    obsdata.f[2] = mxGetPr(theInput[23]);
    obsdata.err[1] = mxGetPr(theInput[24]);
    obsdata.err[2] = mxGetPr(theInput[25]);
    
    //filenameheader = mxArrayToString(theInput[26]);
    
    //mexPrintf("%s",filenameheader);
	...
	mexPrintf("M = %lf, R = %lf, i = %lf, e = %lf, ts = %lf\n", mass, rspot, incl_1, theta_1, ts);

	// Calculating numtheta based on rho
	//if ( rho <= 4.0 ) numtheta = (unsigned int)std::ceil(rho);
	//else if ( rho <= 15.0 ) numtheta = (unsigned int)std::ceil(rho / 3.0);
	//else numtheta = 5;
	//mexPrintf("\nNumtheta = %lf", numtheta);
	numtheta = 1;
	
	 // writes name of everything_out_file to include the date
	strcpy(everything_out_file, "/Users/jasper/Documents/spot/contours/");
	strcat(everything_out_file, filenameheader);
	strcat(everything_out_file, "_everything_from_spotmex.txt"); // also need to change this filename in init
	//std::cout << "everything_out_file = " << everything_out_file << std::endl;
	
	// writes name of lowchisquared_out_file to include the date
	strcpy(low_out_file, "/Users/jasper/Documents/spot/contours/");
	strcat(low_out_file, filenameheader);
	strcat(low_out_file, "_low_from_spotmex.txt"); // also need to change this filename in init
	...
	/*********************************************/
	...
	else if ( (theta_1 - rho) <= 0.0 ) {
		//std::cout << "Within asymetric spot, pre computations and calls." << std::endl;
		...
		if ( numtheta == 1 ) {
			...
			//std::cout << "Within asymetric spot, pre Chi.cpp calls." << std::endl;
	        ...
	        //std::cout << "Within asymetric spot, pre 'add curves'." << std::endl;
		...
		// of each of those segments, on the dge of the spot, instead of chopping up the area.
...
//    double avgPulseFraction(0.0); // average of pulse fractions across all energy bands
 /*   double sumMaxFlux(0.0), sumMinFlux(0.0);
	...
	*/
    ...
    // prints every (physical) parameter set that gets run by ferret to a file
    FILE * printeverything; 
    printeverything = fopen(everything_out_file, "a+");
    
	//if ( printeverything == NULL ) {
    //    mexErrMsgTxt("\n printeverything had problems. Exiting.\n");
    //}
    //fprintf(printeverything, "i am not in low chisquared.\n");
	fprintf(printeverything,"%f\t%f\t%f\t%f\t%f\t%f\n", Units::nounits_to_cgs(mass, Units::MASS)/Units::MSUN, Units::nounits_to_cgs(rspot, Units::LENGTH )*1.0e-5, incl_1 * 180.0 / Units::PI, theta_1 * 180.0 / Units::PI, ts, chisquared);
	fclose(printeverything);
	//std::cout << "Pre printing to low." << std::endl;

	// prints parameter sets with chisquareds < 200 to a file
	if ( chisquared <= 500.0) { 
		FILE * printlowchisquared; 
    	printlowchisquared = fopen(low_out_file, "a+");
    	//std::cout << "In lowchisquared." << std::endl;
    	//if ( printlowchisquared == NULL ) {
        //	mexErrMsgTxt("\n printlowchisquared had problems. Exiting.\n");
    	//}
		fprintf(printlowchisquared,"%f\t%f\t%f\t%f\t%f\t%f\n", Units::nounits_to_cgs(mass, Units::MASS)/Units::MSUN, Units::nounits_to_cgs(rspot, Units::LENGTH )*1.0e-5, incl_1 * 180.0 / Units::PI, theta_1 * 180.0 / Units::PI, ts, chisquared);
	
		fclose(printlowchisquared);
	}
