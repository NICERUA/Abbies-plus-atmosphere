...
echo "Varying Inclination Angle"
...
input_mesh="mesh/TmeshWide.txt"

gnu_file="output/plots/gplot_${day}_incl_${fluxtypefile}.txt"    # name of the text file storing gnuplot commands
lightcurve="output/plots/lightcurve_${day}_incl_${fluxtypefile}" # the lightcurve graphics output file
tex_file="output/plots/plot_${day}_incl_${fluxtypefile}.tex"     # TeX file output with the parameters in the comment
...
while test "$inclination" -lt 90  # this loop makes all of them EXCEPT FOR THE LAST ONE.
do
	if test ! -d "$out_dir"
	...
	echo -n "'$out_file' u 1:$fluxtype w lines lw 4 title 'i = $inclination degrees', " >> "$gnu_file"
...
echo "Output sent to $out_file"
"$exe_dir/spot" -m "$mass" -r "$radius" -f "$spin" -i "$inclination" -e "$emission" -l "$phaseshift" -n "$numbins" -q "$NS_model" -o "$out_file" -p "$rho" -T "$temp" -D "$distance" -t "$numtheta" -u "$E1Lower" -U "$E1Upper" -v "$E2Lower" -V "$E2Upper" -N 
echo -n "'$out_file' u 1:$fluxtype w lines title 'i = $inclination degrees' lw 4 linecolor rgb 'orange';" >> "$gnu_file"
...
echo "\includegraphics[width=1.0\textwidth]{lightcurve_${day}_incl_${fluxtypefile}.eps}" >> "$tex_file"
echo "\caption[Varying inclination angle]{\label{fig:varincl} X-ray light curve for a spherical neutron star with mass \($mass\Msun\), radius $radius~km, spin frequency $spin Hz, varying inclination angle, spot emission angle \($emission\degrees\),  phase shift $phaseshift, spot angular radius \($rho\degrees\), $numtheta \(\times\) $numtheta spot mesh, and spot temperature $temp keV. First energy band $E1Lower-$E1Upper keV, second energy band $E2Lower-$E2Upper keV.}" >> "$tex_file"
