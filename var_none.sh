...
if test "$fluxtype" -eq 4
	then echo "set label 'Low energy band, $E1Lower-$E1Upper keV' at 0.98,0.25 right;" >> "$gnu_file"
fi
if test "$fluxtype" -eq 5 
	then echo "set label 'High energy band, $E2Lower-$E2Upper keV' at 0.98,0.65 right;" >> "$gnu_file"
fi

echo -n "plot [] [0.1:2.2] " >> "$gnu_file"
...
echo -n "'$out_file' u 1:$fluxtype w lines lw 6 linecolor rgb 'magenta'; " >> "$gnu_file"
...
#echo -n "'$out_file' u 1:$fluxtype w lines title 'Ideal' lw 4 linecolor rgb 'orange', " >> "$gnu_file"
#echo -n "'$in_file_to_plot' u 1:2:3 w yerrorbars title 'Data' lw 4 linecolor rgb 'blue';" >> "$gnu_file"
...
echo "\includegraphics[width=4.5in]{lightcurve_${day}_${fluxtypefile}.eps}" >> "$tex_file"
