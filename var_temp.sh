...
emission=60000e-3 # in degrees
rho=6000e-3  # in degrees
...
gnu_file="output/plots/gplot_${day}_temp_${fluxtypefile}.txt"    # name of the text file storing gnuplot commands
lightcurve="output/plots/lightcurve_${day}_temp_${fluxtypefile}" # the lightcurve graphics output file
tex_file="output/plots/plot_${day}_temp_${fluxtypefile}.tex"     # TeX file output with the parameters in the comment
...
out_dir="output"
out_file="$out_dir/q$NS_model/m$em/r$radius/i$inclination/e$spotangle/f$spin/p$rho/T$temp/n$num"
...
while test "$temp" -lt 20  # this loop makes all of them EXCEPT FOR THE LAST ONE.
do
	if test ! -d "$out_dir"
	...
	echo -n "'$out_file' u 1:$fluxtype w lines lw 2 title 'T = $temp keV', " >> "$gnu_file"
	let temp=temp+5
	out_dir="output/q$NS_model/m$mass/r$radius/f$spin/i$inclination/e$emission/p$rho/T$temp/n$numtheta"
	out_file="${out_dir}"/"${day}"_"${gray_type}".txt
...
echo "\caption{X-ray light curve for $NSshape neutron star with mass $mass \Msun, radius $radius km, inclination angle $inclination degrees, spot angle $emission degrees, NS spin frequency $spin Hz, phase shift $phaseshift, distance to NS $distance m, spot angular radius $rho degrees, $numtheta x $numtheta spot mesh, and varying spot temperature. First energy band $E1Lower eV to $E1Upper eV, second energy band $E2Lower eV to $E2Upper eV.}" >> "$tex_file"
