...
echo "Varying NS Spin Frequency"
# integers
...
gnu_file="$exe_dir/output/plots/gplot_${day}_spin_${fluxtypefile}.txt"    # name of the text file storing gnuplot commands
lightcurve="$exe_dir/output/plots/lightcurve_${day}_spin_${fluxtypefile}" # the lightcurve graphics output file
tex_file="$exe_dir/output/plots/plot_${day}_spin_${fluxtypefile}.tex"     # TeX file output with the parameters in the comment
...
echo -n "'$out_file' u 1:$fluxtype w lines lw 2 title 'f = $spin Hz', " >> "$gnu_file"
...
while test "$spin" -lt 800  # this loop makes all of them EXCEPT FOR THE LAST ONE.
do
	if test ! -d "$out_dir"
    	then mkdir -p "$out_dir"
	fi
...
echo "\caption{X-ray light curve for $NSshape neutron star with mass $mass \Msun, radius $radius km, inclination angle $inclination degrees, spot angle $emission degrees, phase shift $phaseshift, distance to NS $distance m, spot angular radius $rho degrees, $numtheta x $numtheta spot mesh, spot temperature $temp keV, and varying NS spin frequency. First energy band $E1Lower eV to $E1Upper eV, second energy band $E2Lower eV to $E2Upper eV.}" >> "$tex_file"
