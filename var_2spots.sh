...
fluxtype=2 # ALSO NEED TO CHANGE WHAT COLUMN OF INPUT FILE GETS PRINTED, ~ LINE 150 HERE
...
gnu_file="$exe_dir/output/plots/gplot_${day}_2spots_i${inclination}_${fluxtypefile}.txt"    # name of the text file storing gnuplot commands
lightcurve="$exe_dir/output/plots/lightcurve_${day}_2spots_i${inclination}_${fluxtypefile}" # the lightcurve graphics output file
tex_file="$exe_dir/output/plots/plot_${day}_2spots_i${inclination}_${fluxtypefile}.tex"     # TeX file output with the parameters in the comment
...
if test "$fluxtype" -eq 2
	then echo "set label 'Bolometric' at 0.98,0.01 right;" >> "$gnu_file"
fi
if test "$fluxtype" -eq 3
...
## RUNNING THE CODE
"$exe_dir/spot" -m "$mass" -r "$radius" -f "$spin" -i "$inclination" -e "$emission" -l "$phaseshift" -n "$numbins" -q "$NS_model" -o "$out_file" -p "$rho" -T "$temp" -D "$distance" -t "$numtheta" -u "$E1Lower" -U "$E1Upper" -v "$E2Lower" -V "$E2Upper" -g "$gray" 

# Plotting just the output file
echo -n "'$out_file' u 1:$fluxtype w lines title 'First spot' lw 4 lt 2 linecolor rgb 'magenta', " >> "$gnu_file"


out_dir="$exe_dir/output/q$NS_model/m$mass/r$radius/f$spin/i$inclination/e$emission/p$rho/T$temp/n$numtheta"
out_file="${out_dir}"/"${day}"_"${gray_type}"_second.txt
#out_file="${out_dir}/q${NS_model}_m${mass}_r${radius}_f${spin}_i${inclination}_e${emission}_p${rho}_T${temp}_n${numtheta}"
...
## RUNNING THE CODE
"$exe_dir/spot" -j -m "$mass" -r "$radius" -f "$spin" -i "$inclination" -e "$emission" -l "$phaseshift" -n "$numbins" -q "$NS_model" -o "$out_file" -p "$rho" -T "$temp" -D "$distance" -t "$numtheta" -u "$E1Lower" -U "$E1Upper" -v "$E2Lower" -V "$E2Upper" -g "$gray" 

# Plotting just the output file
echo -n "'$out_file' u 1:$fluxtype w lines title 'Second spot' lw 4 lt 4 linecolor rgb 'blue', " >> "$gnu_file"

out_dir="$exe_dir/output/q$NS_model/m$mass/r$radius/f$spin/i$inclination/e$emission/p$rho/T$temp/n$numtheta"
out_file="${out_dir}"/"${day}"_"${gray_type}"_both.txt
#out_file="${out_dir}/q${NS_model}_m${mass}_r${radius}_f${spin}_i${inclination}_e${emission}_p${rho}_T${temp}_n${numtheta}"
...
## RUNNING THE CODE
"$exe_dir/spot" -2 -m "$mass" -r "$radius" -f "$spin" -i "$inclination" -e "$emission" -l "$phaseshift" -n "$numbins" -q "$NS_model" -o "$out_file" -p "$rho" -T "$temp" -D "$distance" -t "$numtheta" -u "$E1Lower" -U "$E1Upper" -v "$E2Lower" -V "$E2Upper" -g "$gray" 

# Plotting just the output file
echo -n "'$out_file' u 1:$fluxtype w lines lw 4 lt 1 title 'Both spots'; " >> "$gnu_file"
...
#echo -n "'$out_file' u 1:$fluxtype w lines title 'Simulation' lw 4 linecolor rgb 'magenta', " >> "$gnu_file"
#echo -n "'$in_file_to_plot' u 1:2 w lines title 'Data' lw 4 linecolor rgb 'blue';" >> "$gnu_file"
...
echo "\includegraphics[width=4in]{lightcurve_${day}_2spots_i${inclination}_${fluxtypefile}.eps}" >> "$tex_file"
echo "\caption[Two hot spots, high inclination]{\label{fig:2spots_i${inclination}} X-ray light curve for two antipodal hot spots on $NSshape neutron star with mass \($mass\Msun\), radius $radius~km, inclination angle \($inclination\degrees\), spot emission angle \($emission\degrees\), spin frequency $spin Hz, phase shift $phaseshift, distance $distance m, spot angular radius \($rho\degrees\), $numtheta \(\times\) $numtheta spot mesh, and spot temperature $temp keV. First energy band $E1Lower-$E1Upper keV, second energy band $E2Lower-$E2Upper keV.}" >> "$tex_file"
